...
function ContactView({ href, text, imageSource }) {
  if(!href) return null;
  return(
    <View className={`py-1 flex-row items-center gap-2`}>
      <Image source={imageSource} contentFit='contain' tintColor='white' className={`size-5`} />
      <Link href={href} target='_blank' className={`text-lg font-albertRegular text-white hover:underline`}>{text}</Link>
    </View>
  );
}
  ...
  return (
    <BlurView intensity={50} className={`w-full h-full justify-center items-center ${!appData.isMaximized && 'rounded-b-xl'}`}>
      <View className={`max-w-full max-h-full justify-center items-center`}>
        <ScrollView className={`px-6 py-4 gap-4 rounded-lg bg-black/90`}>
          <ContactView href={AUTHOR_DATA.aboutMe} text={AUTHOR_DATA.fullName} imageSource={require('@/assets/other/user.png')} />
